...
export default function EditPlaceListing({ navigation, route }) {
    const { placeDetails } = route.params;
    const [categories, setCategories] = useState([]);
    const [showCategories, setShowCategories] = useState(false);
    const [isLoading, setIsLoading] = useState(true);
    const [isRetrying, setIsRetrying] = useState(true);
    // Initialize formData with placeDetails
    const [formData, setFormData] = useState({
        name: placeDetails.name || '',
        categoryId: placeDetails.category?.name || 'Category',
        location: `${placeDetails.location?.state}, ${placeDetails.location?.country}` || '',
        openingHour: placeDetails.openingHour || '',
        closingHour: placeDetails.closingHour || '',
        weekendOpeningHour: placeDetails.weekendOpeningHour || '',
        weekendClosingHour: placeDetails.weekendClosingHour || '',
        description: placeDetails.description || '',
        locationImages: placeDetails.locationImages.map(image => image.image_url) || [], // Extract image URLs
        pdf: '', // Initialize as empty, will be populated later
        isReservationEnabled: placeDetails.isReservationEnabled ?? true,
        reservationOptions: placeDetails.reservationOptions || {},
        instagramLink: placeDetails.instagramLink || '',
        xLink: placeDetails.xLink || '',
        tiktokLink: placeDetails.tiktokLink || '',
        isHotel: placeDetails.category?.name === 'Hotel',
        hotelRooms: placeDetails.hotelRooms || []
    });
    ...
    // Initialize rooms state with existing hotel rooms
    const [rooms, setRooms] = useState(
        placeDetails.hotelRooms?.map(room => ({
            name: room.roomName,
    ...
    // Populate reservations based on reservationOptions
    useEffect(() => {
        if (placeDetails.reservationOptions) {
            const initialReservations = Object.keys(placeDetails.reservationOptions);
            setReservations(initialReservations);
            setReservationPrices(placeDetails.reservationOptions);
        }
    }, [placeDetails]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (formData.description.length < 20) validationErrors.push('Description must be at least 20 characters long');
            ...
            // Prepare location parsing
            const [state, country] = formData.location.split(',').map(item => item.trim());

            // Construct payload with careful mapping
            // Prepare the payload
            const payload = {
            ...
            // Assuming you have a way to get existing image IDs, add them here
            if (placeDetails.locationImages) {
                payload.locationImagesIds = placeDetails.locationImages.map(image => image.id);
            }

            // Check if changeLocationImage should be true
            if (payload.locationImages.length === 0) {
                payload.changeLocationImage = false; // No new images, so don't change existing ones
            } else if (payload.locationImages.length !== payload.locationImagesIds.length) {
                // If there are new images but not enough IDs, set changeLocationImage to false
                payload.changeLocationImage = false;
            }

            // Log the payload to verify all images are included
            console.log('Update Payload:', JSON.stringify(payload, null, 2));

            // Make update request with comprehensive error handling
            ...
            if (error.response?.data?.message) {
                ...
                if (Array.isArray(error.response.data.message)) {
                    ...
                    errorMessage = error.response.data.message; // Use the string message directly
                }
            } else {
    ...
    const pickImage = async (index) => {
        try {
            ...
            if (!result.canceled && result.assets?.[0]?.uri) {
                // Ensure we're working with a string URI
                const imageUri = result.assets[0].uri.toString();
    ...
    // Mapping of reservation keys to display text
    const reservationTextMap = {
        reservationRegular: 'Regular',
        reservationVip: 'VIP',
        reservationVvip: 'VVIP',
    };
    ...
    const deleteImageFromServer = async (imageId) => {
        try {
            ...
            const response = await api.delete(`/merchants/${storedMerchantId}/places/${placeDetails.id}/images/${imageId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
    ...
    const deleteImage = async (index) => {
        const imageId = placeDetails.locationImages[index].id; // Get the ID of the image to delete
        const success = await deleteImageFromServer(imageId); // Call the delete function

        if (success) {
            setFormData(prev => {
                const newImages = [...prev.locationImages];
                newImages[index] = null; // Set the deleted image to null
                return { ...prev, locationImages: newImages };
            });
        }
    };
    ...
    const renderImageUpload = (index, isMain = false) => {
        ...
        const imageUri = formData.locationImages[index]; // Get the image URI from formData
        const hasImage = Boolean(imageUri) && typeof imageUri === 'string'; // Ensure it's a string

        return (
            <TouchableOpacity
                style={[containerStyle, hasImage && { borderStyle: 'solid' }]}
                onPress={() => pickImage(index)} // Function to pick an image
            >
                {hasImage ? (
                    <View style={styles.imageContainer}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => deleteImage(index)} // Function to delete the image
                        >
                            <Trash size={isMain ? 24 : 16} color="white" />
                        </TouchableOpacity>
                    </View>
                ) : (
                    <Add size={isMain ? 24 : 20} color="#666" /> // Show add icon if no image
                )}
            </TouchableOpacity>
        );
    };
    ...
    const SOCIAL_MEDIA_PLATFORMS = [
        { id: 'instagram', name: 'Instagram', key: 'instagramLink', icon: Instagram },
        {
            id: 'twitter', name: 'Twitter', key: 'xLink', icon: ({ size, color }) => (
                <FontAwesome6 name="x-twitter" size={size} color={color} />
            )
        },
        {
            id: 'tiktok', name: 'TikTok', key: 'tiktokLink', icon: ({ size, color }) => (
                    ...
                    {isLoading ? (
                        ...
                        categories.map((category) => (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    setFormData(prev => ({ ...prev, categoryId: category.name }));
            ...
            <ScrollView style={styles.content}>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.imageUploadContainer}>
                        {renderImageUpload(0, true)} {/* Main image upload */}
                        <View style={styles.thumbnailContainer}>
                            {[1, 2, 3].map((_, index) => (
                                <React.Fragment key={`image-upload-${index + 1}`}>
                                    {renderImageUpload(index + 1)} {/* Thumbnail uploads */}
                                </React.Fragment>
                            ))}
                        </View>
                    </View>
                ...
                {/* {formData.categoryId === 'Hotel' && (
                    <View style={styles.section}>
                        <Text style={styles.label}>Room category (for hotels)</Text>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.reservationContainer}>
                        {Object.keys(formData.reservationOptions).map((reservation) => (
                            <TouchableOpacity
                                key={`reservation-option-${reservation}`}
                            ...
                            >
                                <Text style={reservations.includes(reservation) ? styles.selectedReservationText : styles.reservationButtonText}>
                                    {reservationTextMap[reservation] || reservation}
                                </Text>
                        ...
                        {reservations.map((reservation) => (
                            <View key={`reservation-price-${reservation}`} style={styles.reservationPriceContainer}>
                                <Text style={styles.reservationPriceLabel}>{reservationTextMap[reservation] || reservation}</Text>
                                <TextInput
                ...
                <View style={styles.section}>
                    ...
                    {SOCIAL_MEDIA_PLATFORMS.map((platform) => (
                        <View key={`social-input-${platform.id}`} style={styles.socialMediaInput}>
                            <TextInput
                            ...
                            <Text style={styles.socialMediaIcon}>
                                {typeof platform.icon === 'function'
                                    ? platform.icon({ size: 20, color: "#AD52F7" })
                                    : <platform.icon size={20} color="#AD52F7" />
                                }
                            </Text>
                        </View>
